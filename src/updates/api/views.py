...
from src.cfeapi.mixins import HttpResponseMixin

from src.updates.forms import UpdateModelForm
from src.updates.models import Update as UpdateModel

from src.updates.api.mixins import CSRFExemptMixin
from src.updates.api.utils import is_json
...
class UpdateModelDetailAPIView(HttpResponseMixin, CSRFExemptMixin, View):
    ...
    def put(self, request, id, *args, **kwargs):
        ...
        #     data[key] = value
        data = {**data, **passed_data}  # merge passed_data dict into data dict
    ...
    def delete(self, request, id, *args, **kwargs):
        ...
        if obj is None:
            error_data = json.dumps({"message": "Update not found"})
            return self.render_to_response(error_data, status=404)
        deleted_, deleted_item = obj.delete()
        if deleted_ == 1:
...
class UpdateModelListAPIView(HttpResponseMixin, CSRFExemptMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if passed_id is not None:  # if id field is in request body, retrieve detail item.
            ...
            json_data = obj.serialize()
            return self.render_to_response(json_data)
        else:  # if id field is not in request body, return queryset.
    ...
    def put(self, request, *args, **kwargs):
        ...
        if not passed_id:
            error_data = json.dumps({"message": "This is a required field to update an item"})
            return self.render_to_response(error_data, status=400)
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if not passed_id:
            error_data = json.dumps({"message": "ID is required to delete an item"})
            return self.render_to_response(error_data, status=400)
